...
@section('title')
    {{ __('admin.coupons.edit.title') }}
@endsection
@section('content')
    <div class="row mb-4 align-items-center justify-content-between">
        <div class="col-md-6 col-sm-12 mb-2 mb-md-0">
            <h2 class="fw-bold text-dark mb-1">{{ __('admin.coupons.edit.title') }}</h2>
        </div>
            ...
            <form action="{{ route('admin.coupons.update', $coupon) }}" method="post">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $coupon->name) }}" maxlength="255" placeholder="{{ __('admin.coupons.name_placeholder') }}">
                    </div>
                    <div class="col-md-6">
                        <label for="code" class="form-label">{{ __('admin.coupons.code') }} <span class="text-danger">*</span></label>
                        <input type="text" name="code" id="code" class="form-control text-uppercase" value="{{ old('code', $coupon->code) }}" required maxlength="64">
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="type" id="type" class="form-select" required>
                            <option value="percentage" {{ old('type', $coupon->type) == 'percentage' ? 'selected' : '' }}>{{ __('admin.coupons.percentage') }}</option>
                            <option value="fixed" {{ old('type', $coupon->type) == 'fixed' ? 'selected' : '' }}>{{ __('admin.coupons.fixed') }}</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <small class="text-muted" id="value_hint">{{ $coupon->type === 'percentage' ? __('admin.coupons.percentage') . ': 1-100' : __('admin.coupons.fixed') }}</small>
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="usage_limit" id="usage_limit" class="form-control" value="{{ old('usage_limit', $coupon->usage_limit) }}" min="1" placeholder="{{ __('admin.coupons.unlimited') }}">
                    </div>
                    <div class="col-md-6">
                        <label for="valid_from" class="form-label">{{ __('admin.coupons.valid_from') }}</label>
                        <input type="datetime-local" name="valid_from" id="valid_from" class="form-control" value="{{ old('valid_from', $coupon->valid_from?->format('Y-m-d\TH:i')) }}">
                    </div>
                    <div class="col-md-6">
                        <label for="valid_until" class="form-label">{{ __('admin.coupons.valid_until') }}</label>
                        <input type="datetime-local" name="valid_until" id="valid_until" class="form-control" value="{{ old('valid_until', $coupon->valid_until?->format('Y-m-d\TH:i')) }}">
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <div class="border rounded p-3 bg-light">
                            @forelse($packages as $pkg)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="package_ids[]" id="pkg_{{ $pkg->id }}" value="{{ $pkg->id }}" {{ in_array($pkg->id, old('package_ids', $coupon->package_ids ?? [])) ? 'checked' : '' }}>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <textarea name="description_ar" id="description_ar" class="form-control" rows="2" maxlength="500">{{ old('description_ar', $coupon->description_ar) }}</textarea>
                    </div>
                    <div class="col-md-6">
                        <label for="description_en" class="form-label">{{ __('admin.coupons.description_en') }}</label>
                        <textarea name="description_en" id="description_en" class="form-control" rows="2" maxlength="500">{{ old('description_en', $coupon->description_en) }}</textarea>
                    </div>
                </div>
                <div class="mb-3">
                    <div class="form-check">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" id="is_active" value="1" class="form-check-input" {{ old('is_active', $coupon->is_active) ? 'checked' : '' }}>
