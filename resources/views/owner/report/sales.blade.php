...
@section('content')
    ...
    <div class="row g-3 mb-4">
        ...
        @include('owner.components.stat-card', [
            ...
            'value' =>
                '<span id="summary_total_revenue" class="num">0</span> <span class="unit">' .
                app('view')->make('components.riyal-icon', ['size' => 'sm'])->render() .
                '</span>',
            'icon' => 'fas fa-coins',
        ...
        <div class="tab-pane fade show active" id="allTab">
            <div class="row mb-3">
                <div class="col-md-3">
                    ...
                    <input type="date" id="start_date" class="form-control" value="{{ $monthStart }}"
                        data-default="{{ $monthStart }}">
                </div>
                <div class="col-md-3">
                    <label for="end_date">{{ __('owner.sales_report.to_date') }}:</label>
                    <input type="date" id="end_date" class="form-control" value="{{ $monthEnd }}"
                        data-default="{{ $monthEnd }}">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-2 d-flex align-items-end gap-2">
                    <button id="filterBtn" class="btn btn-primary btn-sm"
                        style="min-width: 60px;">{{ __('owner.sales_report.filter') }}</button>
                    <button id="resetBtn" class="btn btn-secondary btn-sm"
                        style="min-width: 60px;">{{ __('owner.sales_report.reset') }}</button>
                </div>
                ...
                ajax: {
                    ...
                    dataSrc: function(json) {
                        ...
                        if (json.total_revenue !== undefined) {
                            $('#summary_total_revenue').text(parseFloat(json.total_revenue)
                        ...
                        if (json.net_owner_amount !== undefined) {
                            $('#summary_net_owner').text(parseFloat(json.net_owner_amount)
                                .toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2
                                }));
