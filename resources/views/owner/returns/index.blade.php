...
<div class="d-flex align-items-center mb-3">
    ...
    <div class="ms-auto">
        <a href="#modalCreate" data-bs-toggle="modal" class="btn btn-outline-theme"><i
                class="fa fa-plus-circle btn-success fa-fw me-1"></i>{{__('owner.returns.create.title')}}</a>
        {{-- Print button: open printable report in new tab --}}
        <form action="{{ route('owner.returns.print') }}" method="GET" target="_blank" class="d-inline ms-2">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__('owner.returns.create.title')}}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{route('owner.returns.store')}}" id="createForm" method="post"
                    ...
                    <div class="row">
                        ...
                        <div class="col-6 ">
                            ...
                            <div class="form-group ">
                                <label for="name_ar" class="form-label">{{__('owner.returns.name_ar')}}<span
                                        class="text-danger">*</span></label>
                                <input type="text" name="name_ar" value="{{old('name_ar')}}"
                                    class="form-control  " required
                                    placeholder="{{__('owner.returns.name_ar')}}">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                <label for="name_en" class="form-label">{{__('owner.returns.name_en')}}<span class="text-danger">*</span></label>
                                <input type="text" name="name_en" value="{{ old('name_en') }}" class="form-control" placeholder="{{__('owner.returns.name_en')}}">
                        ...
                        <div class="col-6 ">
                            <div class="form-check form-switch " style="margin-top: 35px">
                                ...
                                <label class="form-check-label" for="status">{{__('owner.returns.activate')}}</label>
...
<div class="modal fade" id="editModel">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{__('owner.returns.edit.title')}}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{route('owner.returns.update','update')}}" id="editForm"
                method="post"
                enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body justify-content-center">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                <label for="name_en" class="form-label">{{__('owner.returns.name_en')}}<span class="text-danger">*</span></label>
                                <input type="text" name="name_en" value="{{ old('name_en') }}" id="name_en" class="form-control" placeholder="{{__('owner.returns.name_en')}}">
                                @error('name_en') <span class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                        </div>
                        <div class="col-6 ">
    ...
    function deleteRecord(recordId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('owner/returns') }}/" + recordId,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
