...
@section('content')
    ...
    <div class="d-flex align-items-center mb-3">
        <div>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">{{ __('owner.generated.manage_crew') }}</a></li>
                <li class="breadcrumb-item active">{{ __('owner.generated.edit_crew') }}</li>
            </ul>
            <h1 class="page-header mb-0">{{ __('owner.generated.edit_crew') }}</h1>
        </div>
    ...
    <div id="formControls" class="mb-5">
        <div class="card">
            <div class="card-body pb-2">
                <form action="{{ route('owner.crew.update', $data->id) }}" id="editForm" method="post"
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="captain_id" class="form-label">{{ __('owner.sales.boat') }}<span
                                ...
                                <select name="boat_id" class="form-control" required id="boat_id">
                                    ...
                                    @foreach ($boats as $boat)
                                        <option value="{{ $boat->id }}"
                                            {{ old('boat_id', $data->boat_id) == $boat->id ? 'selected' : '' }}>
                                            {{ $boat->name_ar }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        @php
                            $hasAttachment = !empty($data->attachment ?? null);
                            $hasIdAttachment = !empty($data->id_attachment ?? null);
                        @endphp

                        <div class="col-md-4 saudi-fields">
                            <label class="form-label">{{ __('owner.crew.edit.id_attachment') }}<span
                                    class="text-danger">*</span></label>
                            <input type="file" name="id_attachment" class="form-control"
                                data-has-attachment="{{ $hasIdAttachment ? 'true' : 'false' }}"
                                @if (!$hasIdAttachment) required @endif>
                            @if ($hasIdAttachment)
                                <br>
                                <a href="{{ asset($data->id_attachment) }}" target="_blank"
                                    class="btn btn-sm btn-outline-primary mt-1">{{ __('owner.expenses.show.view_attachment') }}</a>
                            @endif
                            @error('id_attachment')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="col-md-4 non-saudi-fields">
                            ...
                            <input type="file" name="attachment" class="form-control"
                                ...
                                @if (!$hasAttachment) required @endif>
                            @if ($hasAttachment)
                                <br>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="salary_type" required class="form-control">
                                <option value="">{{ __('owner.crew.edit.select_placeholder') }}</option>
                                <option value="salary"
                                    {{ old('salary_type', $data->salary_type) == 'salary' ? 'selected' : '' }}>
                                    {{ __('owner.crew.edit.salary_option_salary') }}</option>
                                <option value="percentage"
                                    {{ old('salary_type', $data->salary_type) == 'percentage' ? 'selected' : '' }}>
                                    {{ __('owner.payrolls.show.percentage') }}%
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="fishing_license_number"
                                value="{{ old('fishing_license_number', $data->fishing_license_number) }}"
                                class="form-control"
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" name="fishing_license_expiry"
                                value="{{ old('fishing_license_expiry', $data->fishing_license_expiry) }}"
                                class="form-control" required>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="governorate_id" class="form-control" required id="governorate_id">
                                    <option value="">{{ __('owner.crew.edit.select_placeholder') }}</option>
                                    @if (old('region_id', $data->region_id))
                                        @php
                                            $oldGovernorates = \App\Models\Governorate::where(
                                                'region_id',
                                                old('region_id', $data->region_id),
                                            )->get();
                                        @endphp
                                        @foreach ($oldGovernorates as $gov)
                                            <option value="{{ $gov->id }}"
                                                {{ old('governorate_id', $data->governorate_id) == $gov->id ? 'selected' : '' }}>
                                                {{ $gov->name }}
                                            </option>
                                        @endforeach
                                    @endif
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="port_id" id="port_id" required class="form-control">
                                ...
                                @php
                                    ...
                                    $ports = $selectedGovernorateId
                                        ? \App\Models\Port::where('governorate_id', $selectedGovernorateId)->get()
                                        : collect();
                                @endphp
                        ...
                        <div class="col-xl-4">
                            <div class="form-group ">
                                <label for="password" class="form-label">{{ __('owner.crew.edit.password') }}</label>

                                <input type="password" name="password" value="{{ old('password') }}"
                                    class="form-control  " placeholder="{{ __('owner.crew.edit.password') }}">
                        ...
                        <div class="col-xl-4">
                            <div class="form-group ">
                                <label for="password" class="form-label">
                                    {{ __('owner.crew.edit.password_confirmation') }}</label>
