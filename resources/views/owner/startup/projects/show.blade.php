...
<div class="tab-content card border-top-0 shadow-sm">
                ...
                <tbody>
                    @forelse($project->partners as $partner)
                        <tr>
                            ...
                            @endforeach
                            <td>{{ $partner->notes ?: '—' }}</td>
                            <td><div class="d-flex align-items-center gap-1"><a href="{{ route('owner.startup.partners.edit', $partner) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a><form method="POST" action="{{ route('owner.startup.partners.destroy', $partner) }}" onsubmit="return confirm(@js(__('owner.startup.confirm_delete')))" class="m-0">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></div></td>
                        </tr>
                    @empty
                ...
                <tbody>
                    @forelse($expenses as $expense)
                        <tr>
                            ...
                            <td><div class="d-flex align-items-center gap-1"><a href="{{ route('owner.startup.expenses.edit', $expense) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a><form method="POST" action="{{ route('owner.startup.expenses.destroy', $expense) }}" onsubmit="return confirm(@js(__('owner.startup.confirm_delete')))" class="m-0">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></div></td>
                        </tr>
                    @empty
    ...
    <div id="contributions" class="tab-pane fade @if($activeTab === 'contributions') show active @endif p-3 p-lg-4">
        ...
        <div class="table-responsive"><table class="table align-middle text-center"><thead><tr><th>{{ __('owner.startup.date') }}</th><th>{{ __('owner.startup.partner_name') }}</th><th>{{ __('owner.startup.amount') }}</th><th>{{ __('owner.startup.payment_method') }}</th><th>{{ __('owner.startup.contribution_type') }}</th><th>{{ __('owner.startup.notes') }}</th><th></th></tr></thead><tbody>@forelse($project->contributions as $contribution)<tr><td>{{ $contribution->date->format('Y-m-d') }}</td><td>{{ $contribution->partner->name }}</td><td>{{ number_format((float) $contribution->amount, 2) }}</td><td>{{ __('owner.startup.methods.'.$contribution->payment_method) }}</td><td>{{ __('owner.startup.contribution_types.'.$contribution->type) }}</td><td>{{ $contribution->notes ?: '—' }}</td><td><div class="d-flex align-items-center gap-1"><a href="{{ route('owner.startup.contributions.edit', $contribution) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a><form method="POST" action="{{ route('owner.startup.contributions.destroy', $contribution) }}" onsubmit="return confirm(@js(__('owner.startup.confirm_delete')))" class="m-0">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></div></td></tr>@empty<tr><td colspan="7" class="text-center text-muted py-4">{{ __('owner.startup.empty') }}</td></tr>@endforelse</tbody></table></div>
    </div>
        ...
        <div class="d-grid gap-3">
            @forelse($project->loans as $loan)
                <div class="border rounded p-3">
                    ...
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 border-top border-bottom py-2 mb-2"><div class="small"><span class="text-muted">{{ __('owner.startup.paid') }}:</span> {{ number_format((float) $loan->payments->sum('amount'), 2) }} · <span class="text-muted">{{ __('owner.startup.loans_remaining') }}:</span> {{ number_format(max(0, (float) $loan->amount - (float) $loan->payments->sum('amount')), 2) }}</div><div class="d-flex align-items-center gap-1"><a href="{{ route('owner.startup.loans.edit', $loan) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a><form method="POST" action="{{ route('owner.startup.loans.destroy', $loan) }}" onsubmit="return confirm(@js(__('owner.startup.confirm_delete')))" class="m-0">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></div></div>
                    @if($loan->payments->isNotEmpty())<div class="table-responsive"><table class="table table-sm mb-3 text-center"><thead><tr><th>{{ __('owner.startup.date') }}</th><th>{{ __('owner.startup.amount') }}</th><th>{{ __('owner.startup.payer') }}</th><th>{{ __('owner.startup.payment_method') }}</th><th>{{ __('owner.startup.notes') }}</th><th></th></tr></thead><tbody>@foreach($loan->payments->sortBy('date') as $payment)<tr><td>{{ $payment->date->format('Y-m-d') }}</td><td>{{ number_format((float) $payment->amount, 2) }}</td><td>{{ $payment->payer_type === 'partner' ? ($payment->partner?->name ?? '—') : __('owner.startup.payer_types.project') }}</td><td>{{ __('owner.startup.methods.'.$payment->payment_method) }}</td><td>{{ $payment->notes ?: '—' }}</td><td><form method="POST" action="{{ route('owner.startup.loans.payments.destroy', $payment) }}" onsubmit="return confirm(@js(__('owner.startup.confirm_delete')))" class="m-0">@csrf @method('DELETE')<button class="btn btn-sm p-0 border-0 bg-transparent text-danger"><i class="bi bi-trash"></i></button></form></td></tr>@endforeach</tbody></table></div>@endif
